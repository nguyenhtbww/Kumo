import { url, tygia, IProduct, CProduct } from "./common.js";


export const getBrands = async (limit: Number = 6) => {
  // gọi API và đón nhận từ dữ liệu ré và chuyển về dạng JSON, sau đí then lần nữa trả về data
...
export const getProductDetail = async (product_id: SVGAnimatedNumberList) => {
  ...
  // khai báo biến obj là kiểu lớp CProduct
  let obj: CProduct;
  // tạo đối tượng từ lớp CProduct rồi gán vào biến obj
...
//begin : chức năng hiển thị sản phẩm mới ở trang chủ
export const getProductsByBrand = async (brand_id: number, limit: Number = 50) => {
  // gọi API và đón nhận từ dữ liệu ré và chuyển về dạng JSON, sau đí then lần nữa trả về data
...
//begin : chức năng hiển thị sản phẩm mới ở trang chủ
export const getNewProducts = async (limit: Number = 6) => {
    // gọi API và đón nhận từ dữ liệu ré và chuyển về dạng JSON, sau đí then lần nữa trả về data
...
//begin : chức năng hiển thị sản phẩm mới ở trang chủ
export const getHotProducts = async (limit: Number = 6) => {
    // gọi API và đón nhận từ dữ liệu ré và chuyển về dạng JSON, sau đí then lần nữa trả về data
